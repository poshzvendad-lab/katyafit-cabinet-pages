...
// Куда ведёт «открыть полный доступ»: на карточки тарифов лендинга — там описание
// того, что покупается, и там же оплата. Раньше вело в личку Кате (продажа руками).
const KF_UPGRADE_URL = 'https://katyafit.ru/#rates';
...
const KF_PREVIEW = (() => {
    ...
    // &kg=100&act=high — вес и активность мок-анкеты (?v=0.6.74). Нужны, чтобы
    // посмотреть план участницы с БОЛЬШОЙ нормой: порции и добавочные приёмы
    // видны только там, где норма заметно выше базовых ≈1200 ккал.
    const kg = Math.min(200, Math.max(40, parseFloat(p.get('kg')) || 0));
    const act = ['low', 'medium', 'high'].indexOf(p.get('act')) !== -1 ? p.get('act') : null;
    return { progress: want, seedMeasure: p.has('m'), tier, kg: kg || null, act };
})();
...
// Запись в базу с тихим повтором при обрыве связи. iPhone при переключении
// Wi-Fi/мобильной сети роняет запрос разовым «TypeError: Load failed» (статус 0):
// в журнале ошибок 07.07–14.09.2026 таких 7 карточек, и каждый раз участница
// просто нажимала ещё раз — со второго раза ложилось. Теперь повторяем сами.
// mk() строит запрос ЗАНОВО на каждой попытке (свежие данные из памяти).
// Повторяем только сеть (статус 0) и 5xx. Всё, что идёт через dbWrite, —
// upsert/update/delete по ключу, то есть повтор после «дошло, но ответ
// потерялся» ничего не задвоит. Отказы прав/сессии (4xx) не повторяем.
const DB_RETRY_WAITS = [700, 1500, 2500];   // мс; ~5 с сверху, потом честная ошибка
async function dbWrite(mk) {
    for (let i = 0; ; i++) {
        let r;
        try { r = await mk(); }
        catch (e) { r = { error: { message: String((e && e.message) || e), code: '' }, status: 0 }; }
        if (!r || !r.error) return r || {};
        const net = !r.status || r.status >= 500;
        if (!net || i >= DB_RETRY_WAITS.length) return r;
        await new Promise(res => setTimeout(res, DB_RETRY_WAITS[i]));
    }
}
...
const KF = {
    ...
    _bootPreview() {
        ...
        if (KF_PREVIEW.seedMeasure) {
            ...
            // Вес и активность можно задать (&kg=100&act=high) — так видно план
            // участницы с большой нормой (персональные порции, добавочные приёмы).
            this._measure.start  = { phase: 'start',  age: 32, height: 168, weight: KF_PREVIEW.kg || 72.4, activity: KF_PREVIEW.act || 'medium', chest: 96, waist: 78, belly: 84, hips: 101,  _photoUrl: ph('#b9c4ab', 'фото до') };
    ...
    // Входит ли тренировка дня n в тариф. БЕЗ аргумента — «показываем ли раздел
    // тренировок вообще» (поведение как раньше). На «питании» тренировок нет, но
    // дни 1-3 остаются: питание покупается СВЕРХ пробного (решение 30.07.2026).
    // Настоящий замок — на сервере (0027): вне тарифа workout приходит пустым.
    showsWorkouts(n) { return this.tier !== 'nutrition' || (n != null && n <= 3); },
    ...
    async setHabit(n, idx, val) {
        ...
        const { error } = await dbWrite(() => sb.from('day_logs').upsert({ user_id: uid, day: n, habits: l.habits }, { onConflict: 'user_id,day' }));
        if (error) {
    ...
    async completeDay(n, data) {
        ...
        if (KF_PREVIEW) return { ok: true };
        const { error: e1 } = await dbWrite(() => sb.from('day_logs').upsert(
            { user_id: uid, day: n, completed: true, mood: l.mood, habits: l.habits, completed_at: l.completed_at },
            { onConflict: 'user_id,day' }));
        const { error: e2 } = e1 ? { error: e1 }
            : await dbWrite(() => sb.from('profiles').update({ progress_day: newProg, last_active: t }).eq('id', uid));
        const error = e1 || e2;
        if (error) {
    ...
    async getDay(n) {
        ...
        // Читаем ПРЕДСТАВЛЕНИЕ, а не таблицу: оно вычищает workout там, где
        // тренировки не входят в тариф (миграция 0027). Саму program_days
        // участнице читать нельзя — иначе ссылки на видео утекают.
        const { data } = await sb.from('program_days_v').select('*').eq('day', n).maybeSingle();
            ...
            jobs.push(this._raceNutri(
                ...
                rows => {
                    ...
                    rows.forEach(r => {
                        ...
                        else if (r.id === 'extras') NUTRITION.extras = r.data;
                    });
    ...
    _loadNutritionPreview(opts) {
        ...
        if (opts.reference && !NUTRITION._refLoaded && KF_PREVIEW.tier !== 'none') {
            ...
            NUTRITION.extras = this._previewExtras();
    ...
    _previewNutritionDay(n) {
        ...
        // Названия продуктов — обычные (курица, гречка, творог): они и так
        // перечислены в публичном js/nutrition-scale.js, а платное здесь — КАКОЕ
        // меню в какой день, и оно по-прежнему выдумано. Зато превью проходит те
        // же ветки кода, что прод: персональные порции узнают продукт и растут.
        if (cheat) {
            return { week, theme: 'Превью · читмил', phase: 'Читмил', cheat: true, meals: [
                { slot: 'Завтрак', text: 'Омлет из 2 яиц, овощи.', p: 18, f: 14, c: 6, items: [], rec: ['Демо-шаг рецепта для превью.'] },
                { slot: 'Обед', text: 'Куриное филе (170 г сырой, ≈ 120 г готового), гречка отварная (50 г сухой, ≈ 140 г готовой).', p: 40, f: 8, c: 40, items: [{ n: 'Куриное филе', g: 120, k: 0.7 }, { n: 'Гречка', g: 140, k: 2.7 }], rec: ['Демо-шаг рецепта для превью.'] },
        ...
        return { week, theme: 'Превью-неделя ' + week, phase: 'Демо', cheat: false, meals: [
            { slot: 'Завтрак', text: 'Овсяная каша (45 г сухой, ≈ 135 г готовой), 2 отварных яйца.', p: 22, f: 15, c: 35, items: [{ n: 'Овсяные хлопья', g: 135, k: 3 }], rec: ['Свари кашу — демо-шаг для превью.', 'Отвари яйца — второй шаг.'] },
            { slot: 'Обед', text: 'Треска запечённая (180 г сырой, ≈ 145 г готового), картофель (180 г сырой, ≈ 150 г готового), овощи.', p: 35, f: 9, c: 38, items: [{ n: 'Треска', g: 145, k: 0.8 }, { n: 'Картофель', g: 150, k: 0.83 }], rec: ['Запеки треску — демо-шаг превью.'] },
            { slot: 'Перекус', treat: true, text: 'Горький шоколад (20 г) + чай.', p: 2, f: 8, c: 20 },
            { slot: 'Ужин', text: 'Творог 5% (150 г), ягоды (80 г).', p: 27, f: 10, c: 12, items: [], rec: ['Смешай творог с ягодами — демо-шаг превью.'] },
        ] };
    },
    _previewConstructor() {
    ...
    // Банк добавочных приёмов (подгонка плана под норму, ?v=0.6.74). Размеры
    // разные — движок подбирает под остаток нормы; тексты демонстрационные.
    _previewExtras() {
        return [
            { slot: 'Перекус', after: 'Обед', kcal: 92, p: 2, f: 0, c: 21, text: 'Демо-банан.', rec: ['Демо-шаг: съешь банан.'] },
            { slot: 'Второй завтрак', after: 'Завтрак', kcal: 174, p: 21, f: 6, c: 9, text: 'Демо-творог (120 г) с демо-ягодами (80 г).', rec: ['Демо-шаг: смешай творог с ягодами.'] },
            { slot: 'Перекус', after: 'Обед', kcal: 242, p: 16, f: 14, c: 13, text: 'Демо-бутерброд: демо-хлеб (1 кусок), демо-сыр (25 г), 1 варёное яйцо.', rec: ['Демо-шаг: собери бутерброд.'] },
            { slot: 'Второй завтрак', after: 'Завтрак', kcal: 431, p: 14, f: 15, c: 60, text: 'Демо-каша (50 г сухой) с демо-орехами (15 г) и бананом.', items: [{ n: 'Демо-крупа (добавочная)', g: 150, k: 3 }], rec: ['Демо-шаг: свари кашу.'] },
        ];
    },
    _previewShopping() {
        // Названия — те же, что в мок-меню выше: иначе пересчёт списка под личную
        // норму в превью не срабатывал бы, и проверка показывала не то, что прод.
        const wk = w => ({ week: w, note: 'Демо-примечание списка недели ' + w + '.', groups: [
            { t: 'Мясо, рыба', items: [['Куриное филе', '≈ 1,3 кг'], ['Треска', '≈ 500 г']] },
            { t: 'Крупы, хлеб', items: [['Гречка', '≈ 175 г'], ['Овсяные хлопья', '≈ 90 г'], ['Картофель', '≈ 200 г (2 средних)'], ['Хлеб цельнозерновой', '4 куска']] },
            { t: 'Молочное и яйца', items: [['Яйца', '10 шт'], ['Творог 5%', '≈ 500 г']] },
        ] });
    ...
    async acceptConsent() {
        ...
        const { error } = await dbWrite(() => sb.from('profiles').update(patch).eq('id', uid));
        if (error) {
            const auth = error.code === 'PGRST301' || /jwt|token|401|403|permission/i.test(error.message || '');
    ...
    // участница выбирает в анкете (ACTIVITY_LEVELS); дефицит зашит — позже
    // вынести в настройку, чтобы Катя крутила.
    // Возвращает { bmr, tdee, kcal, protein, fat, carb, activity, calcWeight }
    // или null (нет анкеты).
    nutritionTargets() {
        const m = this._measure.start;
        ...
        // ─── Б/Ж/У (26.07.2026, решение владельца) ───
        // Белок 1.8 г/кг и жиры 1 г/кг считаем НЕ от фактического веса, а от
        // СКОРРЕКТИРОВАННОЙ массы: здоровый вес для роста + 40% лишнего. Так
        // делают при избыточном весе — жировой ткани белок не нужен, а от
        // фактического веса цифры выходят неподъёмными (115 кг → 207 г белка
        // в день) и вместе с жирами не влезают в бюджет: у женщины 100 кг с
        // низкой активностью на углеводы оставалось РОВНО НОЛЬ калорий.
        const ideal = 22 * (h / 100) * (h / 100);           // ИМТ 22 — середина нормы
        const aw = w <= ideal ? w : ideal + 0.4 * (w - ideal);
        const protein = Math.round(1.8 * aw);               // белок 1.8 г/кг — верх нормы при похудении
        // Жиры 1 г/кг, но не выше 35% калорий (AMDR). Без потолка при дефиците
        // жиры съедали бы 40–44% рациона, вытесняя крупы и фрукты.
        const fat = Math.round(Math.min(1 * aw, kcal * 0.35 / 9));
        const carb = Math.max(0, Math.round((kcal - protein * 4 - fat * 9) / 4));
        return { bmr: Math.round(bmr), tdee: Math.round(tdee), kcal, protein, fat, carb, activity: lv, calcWeight: Math.round(aw) };
    },
    ...
    async saveMeasurements(phase, data, photoBlob) {
        ...
        const { data: saved, error } = await dbWrite(() => sb.from('measurements')
    ...
    async reset() {
        ...
        const t = todayISO();
        if (!KF_PREVIEW) {
            const { error: e1 } = await dbWrite(() => sb.from('profiles')
                .update({ progress_day: 1, start_date: t, last_active: t }).eq('id', uid));
            const { error: e2 } = e1 ? { error: e1 }
                : await dbWrite(() => sb.from('day_logs').delete().eq('user_id', uid));
            const { error: e3 } = (e1 || e2) ? { error: e1 || e2 }
                : await dbWrite(() => sb.from('measurements').delete().eq('user_id', uid));
